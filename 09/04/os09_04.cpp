...
int main()  {
    ...
    LPWSTR fileName = (LPWSTR)(L"C:\\Oracle\\OS09_01.txt");
...
BOOL printWatchRowFileTxt(LPWSTR FileName, DWORD mlsec) {
    ...
    try
    {
        HANDLE notif = FindFirstChangeNotificationW(path, false, FILE_NOTIFY_CHANGE_SIZE);
        clock_t t1 = clock();
        clock_t t2 = clock();
        DWORD dwWaitStatus;
        ...
        while (true){
            ...
            case WAIT_OBJECT_0:
            {
                if (FindNextChangeNotification(notif) == FALSE) break;
                    ...
                    int position = 0;
                    int rowCount = 0;
                    HANDLE of = CreateFileW(FileName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
                    ...
                    if (GetFileSizeEx(of, fileSize)) {
                        ...
                        if (ReadFile(of, buf, fileSize->QuadPart, &n, NULL))  {
                            while (buf[position++] != '\0') if (buf[position] == '\n')  rowCount++;
                        }
                    }
                    printf("\n Rows count number changed!!: %d", rowCount);
                    CloseHandle(of);
                }
            }
            }
            t2 = clock();
            if (t2 - t1 > mlsec) break;
            mlsec -= (t2 - t1);
        }
        CloseHandle(notif);
