...
namespace HT
{
	...
	HTHANDLE* Create(int Capacity, int SecSnapshotInterval, int MaxKeyLength, int MaxPayloadLength, const char FileName[512])
	{
		try
		{
			...
			if (!(hm = CreateFileMapping(hf, NULL, PAGE_READWRITE, 0, HTsize, NULL))) throw "create filemapping error";
	...
	HTHANDLE* Open(const char FileName[512])
	{
		try
		{
			...
			if ((hf = CreateFileA(FileName, GENERIC_WRITE | GENERIC_READ, NULL, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)) == INVALID_HANDLE_VALUE)	throw "cannot open file";
			if (!(hm = CreateFileMapping(hf, NULL, PAGE_READWRITE, 0, 0, NULL))) throw "create filemapping error";
			if (!(lp = MapViewOfFile(hm, FILE_MAP_ALL_ACCESS, 0, 0, 0))) throw "mapping file error";
	...
	BOOL Close(const HTHANDLE* hthandle)  //выполнить Snap и закрыть HT - хранилище для использования
	{
		try
		{
			if (hthandle == NULL) throw "close: handle null";
