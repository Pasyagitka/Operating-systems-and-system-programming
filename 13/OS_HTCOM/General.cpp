...
namespace HT
{
	...
	void SetLastError(HTHANDLE* ht, const char* message)
	{
		...
		strcpy_s(ht->LastErrorMessage, strlen(message) + 1, message);
	}
	...
	char* GetName(const char* filename)
	{
		...
		{
			if (filename[i] != '\\' && filename[i] != ' ' && filename[i] != '-' && filename[i] != '.' && filename[i] != ':') name[j++] = filename[i];
		}
		return name;
	}

	char* GetFMapName(const char* filename)
	{
		char name[72];
		strncpy_s(name, GetName(filename), 72);
		return name;
		//return (char*)"Map1";
	}
}